 ...
 */
const UneAction = ({ action, termineAction, supprimeAction }) => (
  <View style={styles.conteneurUneAction}>
    <Text style={styles.texteUneAction}>{action.title}</Text>
    <View style={styles.boutons}></View>
    <BoutonAction
      nom="Terminer"
      estGras={action.isDone}
      onClick={() => termineAction(action)}
    ></BoutonAction>
    <BoutonAction
      nom="Supprimer"
      onClick={() => supprimeAction(action)}
    ></BoutonAction>
  </View>
);
